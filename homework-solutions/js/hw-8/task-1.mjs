...
let map = numbers.map((element, _i, array) => element -= array.length);

let filter = numbers.filter((_el, i, arr) => {
   return false ? i === 0 : arr[i] > arr[i - 1]
}
);
...
let reduce = numbers.reduce((acc, element) => acc += element, 0);
